...
//  uint8_t dat[4]={0};
  ...
  send_out[(*idx)++] = len;
  send_out[(*idx)++] = len>>8;
  send_out[(*idx)++] = tpUserControl->work_mode;

  //INT32_TO_LE_BYTES(tpUserControl->work_mode,dat);
...
void report_chage_work_mode(uint8_t work_mode)
{
  ...
  //报开关
  if(work_mode != tpUserControl->work_mode){
      printf(" work_mode %x   %x \n",tpUserControl->work_mode,work_mode);
      tpUserControl->work_mode = work_mode;
      // 不主动上报改变
      uint8_t  send_out[12]={0};
      uint16_t len =1;
      uint16_t idx =0;
      send_out[idx++] = 0xff;
      send_out[idx++] = CMD_DEVICE_REPORT_STATUS;
      send_out[idx++] = seq++;
      send_out[idx++] = 0x01; // 总包数
      send_out[idx++] = 0x00; // 包序号
      send_out[idx++] = 1;  //DP数量
      ...
      send_out[(idx)++] = len>>8;
      send_out[(idx)++] = tpUserControl->work_mode;
     // report_Attributes(send_out,idx);    //不判断群控上报
      report_agreement(send_out,idx);       //群控不上报
  }
}
...
void report_light_temp(void)
{
  if(emberAfNetworkState() != EMBER_JOINED_NETWORK){
   ...
   sl_zigbee_app_debug_println("report_light_temp : len %d %d \r\n",idx,dp_num);
        ...
        case CMD_GATEWAY_COMMAND:
            ...
            for (i = 0; i < dpNum; i++) {
             ...
             //   for (j = 0; j < dpid_len; j++) {
                    ...
                    {
                      case DP_1_SWTICH:
                       tpUserControl->Onoff = data[dataIndex++];
                      ...
                      case DP_2_Work_Mode:
                        ...
                        if(tpUserControl->work_mode == 0x31){
                            tpUserControl->led_mode = 1;
                        }else if(tpUserControl->work_mode == 0x30){
                            tpUserControl->led_mode = 0;
                        }
                       printf("Work_Mode:  %d %d \n",tpUserControl->work_mode,tpUserControl->led_mode);
                        break;
                      case DP_3_Bright:
                        tpUserControl->led_mode  =0;
                        //小端
                        ...
                        dataIndex+=4;
                     //   tpUserControl->Brightness =(data[dataIndex++]/10);
                      ...
                      case DP_4_TEMP:
                        tpUserControl->led_mode  =0;
                     //   tpUserControl->Color_Temp =data[dataIndex++];
                        //小端
                        tpUserControl->Color_Temp = ((uint32_t)(data[dataIndex] | (data[dataIndex+1] << 8) | (data[dataIndex+2] << 16)) | (data[dataIndex+3] << 24));
                        dataIndex+=4;
                        if(tpUserControl->Color_Temp >COLOR_MAX)
                            tpUserControl->Color_Temp =COLOR_MAX;
                        else if(tpUserControl->Color_Temp < COLOR_MIN){
                            tpUserControl->Color_Temp =COLOR_MIN;
                        }
                      ...
                      case DP_6_SCENE:
                        ...
                        if(tpUserControl->scen_mode <=4)report_light_temp();
                      ...
                      case DP_33_POWER_MEMORY:
                        ...
                        if(tpUserControl->memory.mode == 2){
                            ...
                            if(tpUserControl->memory.Color_H >359)tpUserControl->memory.Color_H=0;
                            dataIndex += 2;
                            tpUserControl->memory.Color_S = ((data[dataIndex] << 8) | data[dataIndex + 1]);
                            if(tpUserControl->memory.Color_S >1000)tpUserControl->memory.Color_S=1000;
                            dataIndex += 2;
                            tpUserControl->memory.Color_V = ((data[dataIndex] << 8) | data[dataIndex + 1]);
                            if(tpUserControl->memory.Color_V >1000)tpUserControl->memory.Color_V=1000;
                            dataIndex += 2;



                            tpUserControl->memory.Brightness = ((data[dataIndex] << 8) | data[dataIndex + 1])/10;
                            if(tpUserControl->memory.Brightness >100)tpUserControl->memory.Brightness=100;

                            dataIndex += 2;
                            tpUserControl->memory.Color_Temp = 2700 + (38 * ( (data[dataIndex] << 8) | (data[dataIndex + 1]) /10) );
                            if(tpUserControl->memory.Color_Temp >6500)tpUserControl->memory.Color_Temp=6500;
                          //  pirntf("Brightness %d  Brightness %d \n",tpUserControl->memory.Brightness,tpUserControl->memory.Color_Temp);
                            dataIndex += 2;
                            if((tpUserControl->memory.Color_V!=0)){   //hsv 数据
